...
export default function Navigation({ fetchURL }) {
  const { setIsOpen, setIsCountrySearchOpen, setIsRequestInfoOpen, position } =
    useCommandMenu();
        ...
        >
          <div
            className={
              ...
              (position === "left" ? "flex-col" : "flex-row")
            }
          >
            <Tooltip>
            ...
            <div
              className={
                "bg-zinc-700 " + (position === "left" ? "h-px w-5" : "w-px h-5")
              }
            ></div>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <a
                  ...
                  target="_blank"
                  className="p-2 hover:bg-zinc-800 focus:bg-zinc-800 transition-colors duration-75 rounded-full outline-none relative group"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 496 512"
                    fill="currentColor"
                    className="size-4 group-hover:scale-110 group-focus:scale-110 duration-75"
                  >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <a
                  href="https://github.com/R4ULtv/live-map/issues/new/choose"
                  target="_blank"
                  className="p-2 hover:bg-zinc-800 focus:bg-zinc-800 transition-colors duration-75 rounded-full outline-none relative group"
                >
                  <FlagIcon className="size-4 group-hover:scale-110 group-focus:scale-110 duration-75" />
                </a>
              </TooltipTrigger>
              <TooltipContent side={position === "left" ? "right" : "top"}>
                Bug Report
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <a
                  ...
                  className="p-2 hover:bg-zinc-800 focus:bg-zinc-800 transition-colors duration-75 rounded-full outline-none relative group"
                >
                  <DocumentTextIcon className="size-4 group-hover:scale-110 group-focus:scale-110 duration-75" />
                </a>
              </TooltipTrigger>
              <TooltipContent side={position === "left" ? "right" : "top"}>
                Blog Post
